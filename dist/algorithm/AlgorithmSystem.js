...
exports.AlgorithmSystem = void 0;
...
class AlgorithmSystem {
    ...
    memory;
    debug;
    constructor(config, memory, debug = false) {
        this.config = config;
        this.memory = memory;
        this.debug = debug;
    }
    ...
    async analyzeCurrentState(input) {
...
`.trim();
    ...
    async identifyGap(current, ideal) {
        // Analyze gap between current and ideal
        const gap = [];
        if (current.includes('Unknown'))
            gap.push('Knowledge gap — need to research/understand');
        if (current.includes('resources'))
            gap.push('Resource gap — need tools or information');
        if (current.includes('dependencies'))
            gap.push('Dependency gap — need to resolve relationships');
        gap.push('Execution gap — need to take action');
        return gap;
    }
    generateTransitions(gap, ideal) {
        const phases = [
    ...
    calculateQuality(transitions, gap) {
    ...
    async recordRun(input, current, ideal, transitions, quality) {
    ...
    parseISC(content) {
        ...
        for (const line of lines) {
            if (line.startsWith('## ')) {
                ...
                if (currentSection === 'principles')
                    isc.principles.push(value);
                else if (currentSection === 'constraints')
                    isc.constraints.push(value);
                else if (currentSection === 'criteria')
                    isc.criteria.push(value);
            }
            else if (line.trim() && !line.startsWith('#') && !inList) {
                if (currentSection === 'problem')
                    isc.problem += line.trim() + ' ';
                else if (currentSection === 'vision')
                    isc.vision += line.trim() + ' ';
...
exports.AlgorithmSystem = AlgorithmSystem;
//# sourceMappingURL=AlgorithmSystem.js.map